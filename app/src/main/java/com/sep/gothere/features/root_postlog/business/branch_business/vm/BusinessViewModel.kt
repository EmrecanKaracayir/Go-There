package com.sep.gothere.features.root_postlog.business.branch_business.vm
...
import com.sep.gothere.features.root_postlog.business.branch_business.adapter.BusinessRecyclerViewAdapter
...
class BusinessViewModel @Inject constructor(
    ...
    private var _adapter: BusinessRecyclerViewAdapter? = null
    val adapter get() = _adapter

    suspend fun getUserVenueVM(
        getVenueRequest: GetVenueRequest
    ) = networkVenueRepository.getUserVenueRP(
        getVenueRequest,
        onFetchLoading = {
        ...
        onFetchSuccess = {
            _adapter = BusinessRecyclerViewAdapter(
    ...
    private fun createSearchWidgetBasicVisitList(data: List<GetVenueResponseData>?): List<BusinessWidgetBasicVisit> {
        val searchWidgetBasicVisitList = ArrayList<BusinessWidgetBasicVisit>()
        if (data == null) return searchWidgetBasicVisitList
        for (venueResponseData in data) {
            searchWidgetBasicVisitList.add(
                BusinessWidgetBasicVisit(
